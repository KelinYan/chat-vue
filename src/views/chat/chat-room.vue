    ...
    <a-layout-sider class="sider"
      ...
      <a-menu theme="dark"
        ...
        <a-menu-item v-for='user in users'
                     :key="user._id">
          <a-icon type="user" />
          <span class="nav-text">{{user.name+(user.id?'':'离线')}}</span>
        </a-menu-item>
      </a-menu>
    </a-layout-sider>
    <a-layout>
...
export default {
  ...
  created () {
    ...
    this.$axios.get('http://localhost:7001/user/login?id=' + this.mineId).then(r => {
      this.myInfo = r.data
      this.init(r)
    })
  },
  ...
  methods: {
    init () {
      ...
      this.$axios.get('http://localhost:7001/user/list').then(r => {
    ...
    sendMsg (to, msg) {
      ...
      this.sockets && this.sockets.emit('chat', JSON.stringify({ from: this.myInfo._id, to: to, msg: msg }))
    },
    updateUser (data) {
